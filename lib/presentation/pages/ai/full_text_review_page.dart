  ...
  Future<void> _runReview() async {
    ...
    try {
      ...
      if (characters.isNotEmpty) {
        materialContext.writeln(
          '角色卡：${characters.map((c) => '${c.name}(${c.role ?? ""})').join('、')}',
        );
      }
      if (settings.isNotEmpty) {
        materialContext.writeln(
          '设定：${settings.map((s) => '${s.name}(${s.category ?? ""})').join('、')}',
        );
      }
      if (hooks.isNotEmpty) {
        materialContext.writeln(
          '伏笔：${hooks.map((h) => '${h.title}(${h.isRevealed ? "已回收" : "未回收"})').join('、')}',
        );
      }

      final prompts = {
        'conflict':
            '你是小说设定审查专家。请检查以下章节内容是否存在设定矛盾、逻辑漏洞、前后不一致的问题。列出每个冲突的具体位置和描述。',
                          ...
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Icon(
                                    _selectedCheck == 'conflict'
                                        ? Icons.warning_amber
                                        : _selectedCheck == 'power'
                                        ? Icons.trending_up
                                        : _selectedCheck == 'hook'
                                        ? Icons.link
                                        : Icons.person,
