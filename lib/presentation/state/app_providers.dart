...
final volumesProvider = FutureProvider.family<List<Volume>, String>((
  ref,
  novelId,
) async {
...
final tomatoPresetsProvider = StateProvider<List<TomatoPreset>>(
  (ref) => allTomatoPresets(),
);
final currentPresetProvider = StateProvider<TomatoPreset?>((ref) => null);

// Tomato Agents
final tomatoAgentsProvider = StateProvider<List<TomatoAgent>>(
  (ref) => [
    TomatoAgent(
    ...
    TomatoAgent(
      ...
      systemPrompt:
          '你是中文去AI味助手。将AI生成的机械化文本转换为自然、有人情味的写作风格。\n\n'
          '核心原则：\n'
...
// Materials
final charactersProvider = StateProvider.family<List<Character>, String>(
  (ref, novelId) => [],
);
final settingCardsProvider = StateProvider.family<List<SettingCard>, String>(
  (ref, novelId) => [],
);
final plotHooksProvider = StateProvider.family<List<PlotHook>, String>(
  (ref, novelId) => [],
);
final referencesProvider =
    StateProvider.family<List<ReferenceMaterial>, String>((ref, novelId) => []);
final settingRemindersProvider =
    StateProvider.family<List<SettingReminder>, String>((ref, novelId) => []);
// V2: Locations, Factions, Items
final locationsProvider = StateProvider.family<List<Location>, String>(
  (ref, novelId) => [],
);
final factionsProvider = StateProvider.family<List<Faction>, String>(
  (ref, novelId) => [],
);
final itemsProvider = StateProvider.family<List<Item>, String>(
  (ref, novelId) => [],
);
final relationshipsProvider =
    StateProvider.family<List<CharacterRelationship>, String>(
      (ref, novelId) => [],
    );
final relationshipPositionsProvider =
    StateProvider.family<Map<String, Offset>, String>((ref, novelId) => {});

final customFoldersProvider = StateProvider<List<CustomMaterialFolder>>(
  (ref) => [],
);
...
// 获取当前有效的AI配置（优先用户配置，否则返回null提示用户配置）
