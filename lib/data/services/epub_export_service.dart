...
class EpubExportService {
  ...
  Future<String> exportNovel({
    ...
    // CSS 样式
    archive.addFile(
      ArchiveFile('OEBPS/style.css', _bookCss.length, utf8.encode(_bookCss)),
    );
    ...
    for (final volume in volumes) {
      ...
      for (final chapter in volumeChapters) {
        ...
        final contentFile = File(
          p.join(projectPath, 'chapters', '${chapter.id}.md'),
        );
        String content = '';
...
.volume-title { text-align: center; font-size: 1.8em; margin: 2em 0; color: #6B4EFF; }''';
  ...
  String _buildContentOpf(
    ...
    for (int i = 0; i < infos.length; i++) {
      buffer.writeln(
        '    <item id="chapter_${i + 1}" href="${infos[i].fileName}" media-type="application/xhtml+xml" />',
      );
    }
    buffer.writeln(
  ...
  String _buildTocNcx(String title, String bookId, List<_ChapterInfo> infos) {
    ...
    for (int i = 0; i < infos.length; i++) {
      ...
      if (info.volumeTitle != currentVolume) {
        ...
        playOrder++;
        buffer.writeln(
          '    <navPoint id="volume_$playOrder" playOrder="$playOrder">',
        );
        buffer.writeln(
          '      <navLabel><text>${_escape(currentVolume)}</text></navLabel>',
        );
      ...
      playOrder++;
      buffer.writeln(
        '      <navPoint id="chapter_$playOrder" playOrder="$playOrder">',
      );
      buffer.writeln(
        '        <navLabel><text>${_escape(info.title)}</text></navLabel>',
      );
      buffer.writeln('        <content src="${info.fileName}" />');
