...
class DocxExportService {
  Future<String> exportNovel({
    ...
    final chapters = selectedChapterIds != null && selectedChapterIds.isNotEmpty
        ? chapterRows
              .where((r) => selectedChapterIds.contains(r['id'] as String))
              .toList()
        : chapterRows.toList();
    ...
    archive.addFile(
      ArchiveFile('_rels/.rels', _relsRelsXml.length, _relsRelsXml.codeUnits),
    );
  ...
  String _buildBodyXml(List<Map<String, dynamic>> paragraphs) {
    ...
    for (final p in paragraphs) {
      ...
      buffer.writeln(
        '<w:p>${_pPr(style)}<w:r>${_rPr(style)}<w:t xml:space="preserve">$text</w:t></w:r></w:p>',
      );
    }
  ...
  String _pPr(String style) {
    switch (style) {
      case 'Title':
        return '<w:pPr><w:jc w:val="center"/><w:spacing w:after="400"/></w:pPr>';
      case 'Heading1':
        return '<w:pPr><w:spacing w:before="360" w:after="200"/></w:pPr>';
      case 'Heading2':
        return '<w:pPr><w:jc w:val="center"/><w:spacing w:before="240" w:after="120"/></w:pPr>';
      default:
        return '<w:pPr><w:spacing w:line="360" w:after="200"/></w:pPr>';
    }
  }
  ...
  String _rPr(String style) {
    switch (style) {
      case 'Title':
        return '<w:rPr><w:sz w:val="56"/><w:szCs w:val="56"/><w:b/></w:rPr>';
      case 'Heading1':
        return '<w:rPr><w:sz w:val="36"/><w:szCs w:val="36"/><w:b/></w:rPr>';
      case 'Heading2':
        return '<w:rPr><w:sz w:val="32"/><w:szCs w:val="32"/><w:b/><w:i/></w:rPr>';
      default:
        return '<w:rPr><w:sz w:val="24"/><w:szCs w:val="24"/><w:rFonts w:eastAsia="宋体" w:cs="宋体"/></w:rPr>';
    }
  }
